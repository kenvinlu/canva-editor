...
export default factories.createCoreController('api::shape.shape', ({ strapi }) => ({
    async findShapes(ctx: Context) {
      try {
        ...
        const [entries, total] = await Promise.all([
          strapi.documents('api::shape.shape').findMany({
            where: filters,
            offset: start,
            limit,
            orderBy: { id: 'asc' },
          ...
          strapi.documents('api::shape.shape').count({ filters }),
        ]);
