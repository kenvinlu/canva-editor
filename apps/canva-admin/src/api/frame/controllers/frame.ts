...
export default factories.createCoreController('api::frame.frame', ({ strapi }) => ({
  async findFrames(ctx: Context) {
    try {
      ...
      const [entries, total] = await Promise.all([
        strapi.documents('api::frame.frame').findMany({
          where: filters,
          offset: start,
          limit,
          orderBy: { id: 'asc' },
        ...
        strapi.documents('api::frame.frame').count({ filters }),
      ]);
      ...
      ctx.throw(500, (error as Error).message || 'Failed to fetch frames');
    }
  },
}));