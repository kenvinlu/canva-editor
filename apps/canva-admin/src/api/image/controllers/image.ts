    ...
    async findImages(ctx: Context) {
      try {
        ...
        const [entries, total] = await Promise.all([
          strapi.documents('api::image.image').findMany({
            where: filters,
            offset: start,
            limit,
            orderBy: { id: 'asc' },
          ...
          strapi.documents('api::image.image').count({ filters }),
        ]);
        ...
        ctx.throw(500, (error as Error).message || 'Failed to fetch images');
      }
    },
  }));
