...
const loadMockData = (): MockData => {
  try {
    const data = fs.readFileSync(path.join(__dirname, 'json/templates.json'), 'utf8');
...
// Import master template data to Strapi
export async function importMasterTemplateData(
  apiUrl: string,
  token: string,
  folderId: number
