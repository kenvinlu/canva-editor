...
export default function sortableContainer(
  ...
  return class WithSortableContainer extends React.Component<{items: Array<ItemType>; [key: string]: any}> {
    ...
    componentDidMount() {
      ...
      this.initializeContainerWithRetry(useWindowAsScrollContainer, 0);
    }
  
    initializeContainerWithRetry = (useWindowAsScrollContainer, retryCount = 0) => {
      const maxRetries = 20; // Try up to 20 times for React 19 async refs
      const container = this.getContainer();

      Promise.resolve(container).then((containerNode) => {
        if (!containerNode) {
          // Container not yet available (React 19 async refs), retry
          if (retryCount < maxRetries) {
            setTimeout(() => {
              this.initializeContainerWithRetry(useWindowAsScrollContainer, retryCount + 1);
            }, 50); // Wait 50ms between retries
          } else {
            console.warn('SortableContainer: Could not find container after', maxRetries, 'retries');
          }
          return;
        }
        
        this.initializeContainer(containerNode, useWindowAsScrollContainer);
      });
    }
  
    initializeContainer = (containerNode, useWindowAsScrollContainer) => {
      if (!containerNode) {
        return;
      }
    ...
    handleStart = (event) => {
      ...
      const node = closest(event.target, (el) => {
    ...
    getContainer() {
      ...
      if (typeof getContainer !== 'function') {
        // If ref isn't set yet, try to find the container in the DOM
        if (!this.elementRef.current) {
          // Try to find the ul element that should be the container
          // This is a fallback for React 19 async refs
          const contextNode = this.wrappedInstance?.current || null;
          if (contextNode) {
            // If we have a context node, try to find the ul
            const ulElement = contextNode.querySelector?.('ul');
            if (ulElement) {
              return ulElement;
            }
          }
    ...
    setRef = (node) => {
      // Always store the DOM node for getContainer
      ...
      // In React 19, refs are set asynchronously, so initialize container when ref is set
      if (node && !this.container) {
        // Container ref was just set, try to initialize
        setTimeout(() => {
          if (this.elementRef.current === node && !this.container) {
            const {useWindowAsScrollContainer} = this.props;
            this.initializeContainer(node, useWindowAsScrollContainer);
          }
        }, 0);
      }
    };
    ...
    render() {
      return (
        <SortableContext.Provider value={this.sortableContextValue}>
          <WrappedComponent ref={this.setRef} {...omit(this.props, omittedProps)} />
